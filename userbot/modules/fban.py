...
@bot.on(poci_cmd(outgoing=True, pattern=r"(d)?fban(?: |$)(.*)"))
async def fban(event):
    ...
    if event.sender_id == fban_id:
        return await event.edit(
            "**Error: Tindakan ini telah dicegah oleh protokol keamanan diri Gojo-Userbot.**"
        )
...
@bot.on(poci_cmd(outgoing=True, pattern=r"unfban(?: |$)(.*)"))
...
@bot.on(poci_cmd(outgoing=True, pattern=r"addf(?: |$)(.*)"))
...
@bot.on(poci_cmd(outgoing=True, pattern=r"delf$"))
...
@bot.on(poci_cmd(outgoing=True, pattern=r"listf$"))
...
@bot.on(poci_cmd(outgoing=True, pattern=r"clearf$"))
