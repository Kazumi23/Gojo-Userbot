...
async def shutdown_bot(event):
    ...
    if BOTLOG_CHATID:
        await event.client.send_message(
            BOTLOG_CHATID,
            "**#SHUTDOWN** \n"
            "**Gojo-Userbot** telah di matikan!\nJika ingin menghidupkan kembali silahkan buka heroku",
        )
    await edit_or_reply(event, "**Gojo-Userbot Berhasil di matikan!**")
...
async def restart_bot(event):
    if event.sender_id in SUDO_USERS:
        return
    await edit_or_reply(event, "**Gojo-Userbot Berhasil di Restart**")
    if BOTLOG_CHATID:
        await event.client.send_message(
            BOTLOG_CHATID, "#RESTART \n" "**Gojo-Userbot Berhasil Di Restart**"
        )
...
async def reedme(event):
    await edit_or_reply(
        ...
        "\n┌❏ [Userbot Repo](https://github.com/Cloder07/Gojo-Userbot/blob/main/README.md)"
...
async def string_is_here(event):
    await edit_or_reply(
        ...
        "**List Bot**\n"
        "\n┌❏ **GET STRING BOT:**"
        "\n├❏ [[ KLIK DISINI ]](https://t.me/Gojostringbot)"
        "\n├❏ **GET API ID & API HASH :**"
        "\n└❏ [[ KLIK DISINI ]](https://my.telegram.org/auth)",
    )
...
CMD_HELP.update(
    {
        ...
        \n\n  •  **Syntax :** `{cmd}repo`\
        \n  •  **Function : **Menampilan link Repository Gojo-Userbot\
        \n\n  •  **Syntax :** `{cmd}string`\
        \n  •  **Function : **Menampilan link String Gojostringbot\
    "
    }
)
