...
@bot.on(poci_cmd(outgoing=True, pattern="install$"))
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(poci_cmd(outgoing=True, pattern=r"psend ([\s\S]*)"))
...
@bot.on(poci_cmd(outgoing=True, pattern=r"uninstall (?P<shortname>\w+)"))
